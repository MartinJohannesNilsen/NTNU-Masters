...
if __name__ == "__main__":
    ...
    data_folder = Path(os.path.abspath("")) / "data" / "train_test"
    dest_folder = Path(os.path.abspath("")) / "data" / "train_test_preprocessed"

    all_fpaths = data_folder.rglob("*.csv")

    for fpath in all_fpaths:
        df = pd.read_csv(fpath, sep="‎", quoting=QUOTE_NONE, engine="python")
        df["text"] = df["text"].map(lambda a: " ".join(_tokenize_with_preprocessing(a)))
