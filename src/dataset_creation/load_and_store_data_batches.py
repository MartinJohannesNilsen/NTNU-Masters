...
def fetch_rows_from_h5(fpath: str, start, chunksize):
    """
    Function to fetch embeddings from file with given step size. Helps alleviate memory constraints with large embeddings sizes
    ...
    with h5py.File(fpath, "r") as f:
        fetched_data = [
            f["date"][start:start+chunksize],
            f["emb_tensor"][start:start+chunksize],
            f["name"][start:start+chunksize],
            f["label"][start:start+chunksize]
        ]
