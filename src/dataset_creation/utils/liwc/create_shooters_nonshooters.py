...
preprocessed = False
data_dir = Path(os.path.abspath(__file__)).parents[2] / "data" / "train_test" / f"new{'_preprocessed' if preprocessed else ''}"
...
for max_len in ["256", "512"]:
    ...
    # Save files
    os.makedirs(data_dir / "shooters_nonshooters", exist_ok=True)
    shooters_df.to_csv(data_dir / "shooters_nonshooters" / f"shooters_{max_len}{'_preprocessed' if preprocessed else ''}.csv", sep="‎", header=True, index=False, date_format="%Y-%m-%d")
    nonshooters_df.to_csv(data_dir / "shooters_nonshooters" / f"nonshooters_{max_len}{'_preprocessed' if preprocessed else ''}.csv", sep="‎", header=True, index=False, date_format="%Y-%m-%d")
