...
""" 
def remove_rows_from_h5(new_fpath, old_fpath, rows_idx, step_size):
    # rows_idx has to be sorted in ascending order

    store = h5py.File(new_fpath, "r+")



    first_time_setup_dataset(store)

    with h5py.File(old_fpath,'r+') as h5f:
        orig_size = h5f["idx"].shape[0]
        for idx in rows_idx:
            ds_idx = h5f["idx"]
            ds_date = h5f["date"]
            ds_emb_tensor = h5f["emb_tensor"]
            ds_name = h5f["name"]
            ds_label = h5f["label"]
            ds_length = h5f["length"]

            prev_shape = ds_idx.shape[0]
            ds_idx[idx:prev_shape-1] = ds_idx[idx+1:prev_shape]
            ds_date[idx:prev_shape-1] = ds_date[idx+1:prev_shape]
            ds_emb_tensor[idx:prev_shape-1] = ds_emb_tensor[idx+1:prev_shape]
            ds_name[idx:prev_shape-1] = ds_name[idx+1:prev_shape]
            ds_label[idx:prev_shape-1] = ds_label[idx+1:prev_shape]
            ds_length[idx:prev_shape-1] = ds_length[idx+1:prev_shape]

            ds_idx.resize(prev_shape-1, axis=0)
            ds_date.resize(prev_shape-1, axis=0)
            ds_emb_tensor.resize(prev_shape-1, axis=0)
            ds_name.resize(prev_shape-1, axis=0)
            ds_label.resize(prev_shape-1, axis=0)
            ds_length.resize(prev_shape-1, axis=0)

        print(f"Done reshaping for idx {rows_idx}")
        print(f"Original size: {orig_size}")
        print(f"New size: {h5f['idx'].shape[0]}") """
