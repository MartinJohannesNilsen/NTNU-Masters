...
def read_h5(fpath: str, col_name = None, start: int = None, chunk_size: int = None, tolist = False) -> Union[list, dict]:
    ...
    with h5py.File(fpath, "r") as f:
        if col_name:
            if col_name in ["idx", "date", "emb_tensor", "name", "label"]:
                ...
                elif col_name in ["idx", "label", "length"]:
            ...
            fetched_data = {
                ...
                "length": list(f["length"][start:start+chunk_size if (start != None and chunk_size != None) else None:None].astype(int))
            }
            if tolist:
...
def create_and_store_embeddings(df: pd.DataFrame, fpath: str, emb_model, step_size: int = 200, max_len: int = 512, pad_pos: str = "tail", emb_type: str = "glove"):
    ...
    def embed_rows_as_numpy(rows):
        ...
        #emb_rows = [(embed_and_pad(row, emb_model=emb_model, max_len=max_len, pad_pos=pad_pos)) for row in text_rows]
        #emb_rows = rows["text"].map(lambda text: embed_and_pad(text, emb_model, max_len=max_len, pad_pos=pad_pos)).values
    ...
    def resize_and_append_datasets(data):
        ...
        for colname in cols:
            ...
            store[colname][-data[colname].shape[0]:] = data[colname]
