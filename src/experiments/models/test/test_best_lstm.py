...
from experiments.models.train.gridsearch_cnn import TextDatasetH5py
...
def train(emb_dim: int, hidden_size: int, dropout: float, num_layers: int, emb_type: str, pad_pos: str, max_len: int, batch_size: int, lr: float):
    ...
    train_set = LSTMTextDatasetH5py(train_path_embs, max_len, pad_pos)
    val_set = LSTMTextDatasetH5py(val_path_embs, max_len, pad_pos)
...
def test_model(model: torch.nn.Module, config: dict):
