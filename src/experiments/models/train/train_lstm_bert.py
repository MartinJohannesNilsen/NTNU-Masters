...
class LSTMTextClassifier(nn.Module):
    ...
    def forward(self, emb_tensors, length):
        ...
        """
        packed_input = pack_padded_sequence(emb_tensors, length, batch_first=True, enforce_sorted=False)
        packed_out, _ = self.lstm(packed_input)
        out, _ = pad_packed_sequence(packed_out, batch_first=True)

        seq_len_idx = [(s-1) for s in length] # Get index for each last lstm node in batch to extract hidden state
        out_forward = out[range(len(out)), seq_len_idx, :self.hidden_size] # Get output of last valid LSTM element, not padding
...
def train(max_len: int = 256, num_epochs: int = 10, batch_size: int = 32, stair_twitter: bool = False, pad_pos: str = "tail"):
    # Read data
    ...
    train_path = base_path / f"train_{stair_twitter_str}_bert_{emb_dim}_{pad_pos}_{max_len}.h5"
    val_path = base_path / f"val_{stair_twitter_str}_bert_{emb_dim}_{pad_pos}_{max_len}.h5"
...
def main(max_len: int, pad_pos: str, batch_size: int = 32, stair_twitter: bool = True, storage_type: str = "h5", num_epochs: int = 10):
    train(max_len=max_len, num_epochs=num_epochs, batch_size=batch_size, stair_twitter=stair_twitter, pad_pos=pad_pos)
    #grid_search_lstm(emb_type=emb_type, max_len=max_len, batch_size=batch_size, stair_twitter=stair_twitter, pad_pos=pad_pos)

if __name__ == "__main__":
    train(max_len=256, pad_pos="tail", batch_size=200, stair_twitter=True, num_epochs=10)