...
def grid_search(config):
    ...
    train_path = base_path / f"train_sliced_stair_twitter_{emb_str}_tail_{config['max_len']}.h5"
    val_path = base_path / f"val_sliced_stair_twitter_{emb_str}_tail_{config['max_len']}.h5"
    ...
    for epoch in range(10):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            outputs = model(inputs, lengths).to(float)
        ...
        with torch.no_grad():
            for i, vdata in enumerate(val_loader, 0):
                ...
                voutputs = model(vinputs, vlengths).to(float)
        ...
        gs_path = f"LSTM_gridsearch_sliced_stair_twitter_{config['emb_type']}_{config['emb_dim']}_{config['max_len']}"
...
def test_best_model(best_result):
    ...
    test_path = base_path / f"test_sliced_stair_twitter_{emb_str}_tail_{config['max_len']}.h5"
...
def main(emb_type: str, max_len: int, num_samples=10, max_num_epochs=10):
