...
class LSTMTextClassifier(nn.Module):
    def __init__(self, emb_wts = None, emb_dim: int = 300, dropout: int = 0.5, hidden_size: int = 128, num_layers: int = 2, liwc_size: int = 122):
        ...
        self.fc = nn.Linear((2*hidden_size) + liwc_size, 1)
        self.dropout = nn.Dropout(p=dropout)
        self.sig = nn.Sigmoid()


    def forward(self, emb_tensors, liwc_tensor, idx_first_hidden, idx_last_hidden):
        ...
        out_reduced = torch.cat((out_forward, out_backwards, liwc_tensor), 1) # Concat for fc layer and final pred
...
def check_emb_start(emb_vec):
    # Checks where the padding starts

    i = 0
    """ print(f"emb_vec.shape: {emb_vec.shape}")
    print(f"emb_vec.shape[0]: {emb_vec.shape[0]}") """

    while i < emb_vec.shape[0]-1: # Don't overshoot index so we do -1
        if sum(emb_vec[i]) > 0:
            break
        i += 1
...
class TextDatasetH5py(Dataset):
    def __init__(self, emb_path, liwc_path, max_len, pad_pos):
...
def train(emb_dim: int, hidden_size: int, dropout: float, num_layers: int, emb_type: str, pad_pos: str, max_len: int, batch_size: int, lr: float):
    ...
    train_set = TextDatasetH5py(train_path_embs, train_path_liwc, max_len, pad_pos)
    val_set = TextDatasetH5py(val_path_embs, val_path_liwc, max_len, pad_pos)
...
def test_model(model, config):
    ...
    test_set = TextDatasetH5py(test_path_embs, test_path_liwc, config["max_len"], config["pad_pos"])
