...
class TextDataset(Dataset):
    def __init__(self, glove_path, ft_path):
        self.data_path = data_path
        self.glove_f = h5py.File(self.data_path, "r")
        self.ft_f
...
class TextClassifier(nn.Module):
    def __init__(self, batch_size: int = None, emb_dim: int = 300, sentence_len: int = 256, filter_sizes = [3,4,5], num_filters = [100,100,100], dropout: int = 0.5):
        ...
        self.glove_conv1d_list = nn.ModuleList([
        ...
        self.ft_conv1d_list = nn.ModuleList([
            nn.Conv1d(in_channels=emb_dim,
                      out_channels=num_filters[i],
                      kernel_size=filter_sizes[i])
            for i in range(len(filter_sizes))
        ...
        self.fc = nn.Linear(np.sum(num_filters*2), 1) # Times 2 to accomodate for both fasttext and glove
        self.dropout = nn.Dropout(p=dropout)
        self.sig = nn.Sigmoid() # Sigmoid to squeeze final vals between 0 and 1 to accomodate for binary class prob

    def forward(self, x_glove, x_ft):
        ...
        x_glove = x.permute(0, 2, 1)
        x_ft = x.permute(0, 2, 1)
        #print(f"size: {x_reshaped.size()}")

        # Apply CNN and ReLU. Output shape: (b, num_filters[i], L_out)
        # Attempt to extract features from both glove and ft representations
        x_glove_conv_list = [F.relu(conv1d(x_reshaped)) for conv1d in self.glove_conv1d_list]
        x_ft_conv_list = [F.relu(conv1d(x_reshaped)) for conv1d in self.ft_conv1d_list]

        # Max pooling. Output shape: (b, num_filters[i], 1)
        x_glove_pool_list = [F.max_pool1d(x_conv, kernel_size=x_conv.shape[2]) for x_conv in x_glove_conv_list]
        x_ft_pool_list = [F.max_pool1d(x_conv, kernel_size=x_conv.shape[2]) for x_conv in x_ft_conv_list]
        
        # Concat all features and pass to fc layer
        # Output shape: (b, sum(num_filters))
        x_glove_fc = torch.cat([x_pool.squeeze(dim=2) for x_pool in x_glove_pool_list], dim=1)
        x_ft_fc = torch.cat([x_pool.squeeze(dim=2) for x_pool in x_ft_pool_list], dim=1)
        x_fc = torch.cat([x_glove_fc, x_ft_fc])
