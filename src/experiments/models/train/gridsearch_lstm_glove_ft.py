...
torch.manual_seed(0) 
def train(embedding_type: str, pad_pos: str = "tail", num_epochs: int = 10, max_len: int = 256, embedding_dim: int = 300, batch_size: int = 32, preprocess_type: str = "glove"):

    # Read data
    base_path = None
    if preprocess_type == "glove":
        base_path = Path(os.path.abspath(__file__)).parents[3] / "dataset_creation" / "data" / "train_test" / "new_preprocessed"
    elif preprocess_type == "fasttext":
    ...
    train_df = pd.read_csv(base_path / f"train_sliced_stair_twitter_{max_len}_preprocessed.csv", sep="‎", quoting=QUOTE_NONE, engine="python")
    test_df = pd.read_csv(base_path / f"test_sliced_stair_twitter_{max_len}_preprocessed.csv", sep="‎", quoting=QUOTE_NONE, engine="python")
    val_df = pd.read_csv(base_path / f"val_sliced_stair_twitter_{max_len}_preprocessed.csv", sep="‎", quoting=QUOTE_NONE, engine="python")
    ...
    train_df["text"] = train_df["text"].map(lambda a: get_padded_ids(a, word_to_idx, pad_pos, max_len))
    val_df["text"] = val_df["text"].map(lambda a: get_padded_ids(a, word_to_idx, pad_pos, max_len))
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_vloss < best_vloss:
            ...
            model_path = str(Path(os.path.abspath(__file__)).parents[1] / "saved_models" / "cnn" / f"model_{embedding_type}_{embedding_dim}_{max_len}_{timestamp}_{epoch_number}")
...
if __name__ == "__main__":
    train(embedding_type="glove", pad_pos="tail", num_epochs=10, max_len=256, embedding_dim=300)
