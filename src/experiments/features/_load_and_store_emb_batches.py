...
def replace_text_with_embedding_no_len(df: pd.DataFrame, emb_type: str = "glove", emb_dim: int = 300, emb_model = None):
    assert emb_type == "glove" or emb_type == "fasttext" or emb_type == "bert", "emb_type not supported!"

    if emb_type == "glove":
        df["text"] = df["text"].map(lambda a: get_glove_word_vectors(a, emb_dim=emb_dim, emb_model=emb_model))

    elif emb_type == "fasttext":
        df["text"] = df["text"].map(lambda a: get_fasttext_word_vectors(a, emb_model=emb_model))

    elif emb_type == "bert":
        #df["text"] = df["text"].map(lambda a: get_bert_word_embeddings(a))
        ...
        df["text"] = df["text"].map(lambda a: get_bert_test(a, extract_features))
...
def create_and_store_embeddings_no_len(df: pd.DataFrame, fpath: str, emb_type: str, step_size: int = 200, emb_dim: int = 300, emb_model = None):
    """
    ...
    def embed_rows_as_numpy(rows):
        ...
        rows = replace_text_with_embedding(rows, emb_type=emb_type, emb_dim=emb_dim, emb_model=emb_model)
        ...
        seq_len = np.array([t.shape[1] for t in rows["text"]], dtype=int)
        ...
        out_cols = {
            ...
            "seq_len" : np.array(rows["seq_len"].values, dtype=int)
        }
    ...
    def first_time_setup_dataset(data):
        ...
        store.create_dataset("seq_len", compression="gzip", data=data["seq_len"], chunks=True, maxshape=(None, ))
    ...
    def resize_and_append_datasets(data):
        ...
        store["seq_len"].resize(store["seq_len"].shape[0] + data["seq_len"].shape[0], axis=0)
        store["seq_len"][-data["seq_len"].shape[0]:] = data["seq_len"]
        ...
        if purpose:
            ...
            if size:
                ...
                if purpose == "hold_out":
                    ...
                    create_and_store_embeddings_no_len(embedding_train_df, out_path / f"{purpose}_{emb_type}_{pad_type}{'_256' if size == '256' else ''}.h5", emb_type.replace("_50", ""), 200, emb_dim = 50 if emb_type == "glove_50" else 300, emb_model=emb_model)    
                else:
...
def create_and_store_all_embs_of_type(dfs, emb_type: str, purpose = None, no_stair_twitter = True):
    """
    Convenience function to perform all steps of creating embs and storing them
    ...
    stair_twitter_str = "no_stair_twitter" if no_stair_twitter else "sliced_stair_twitter"
    base_out_path = f"{purpose}_{stair_twitter_str}_{emb_type}"
