...
if __name__ == "__main__":
    ...
    print(f"Type: glove, train")
    embedding_train_df = train_df.copy()
    create_and_store_embeddings(embedding_train_df, out_path / f"train_sliced_stair_twitter_glove.h5", "glove", 200)        

    print(f"Type: glove, test")
    embedding_test_df = test_df.copy()
    create_and_store_embeddings(embedding_test_df, out_path / f"test_sliced_stair_twitter_glove.h5", "glove", 200)

    print(f"Type: glove, hold out")
    embedding_hold_out_df = hold_out_df.copy()
    create_and_store_embeddings(embedding_hold_out_df, out_path / f"hold_out_test_sliced_stair_twitter_glove.h5", "glove", 200)