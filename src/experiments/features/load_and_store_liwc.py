...
def _create():
    data_dir = Path(os.path.abspath(__file__)).parents[0] / "liwc" / "splits" / "csv"
    out_dir = Path(os.path.abspath(__file__)).parents[0] / "liwc" / "splits" / "h5"
...
def _read():
    file = "src/experiments/features/liwc/h5/2022/LIWC-22 Results - test_sliced_stair_twitter - LIWC Analysis.h5"
    data = read_h5(file)
    return data
