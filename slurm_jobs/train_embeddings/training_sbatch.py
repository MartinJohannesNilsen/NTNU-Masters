        ...
        for padding in paddings:
            for size in sizes:
                for variation in variations:
                    ...
                    out = f"out/train_embeddings/{model}_{variation}_{emb}_{padding}_{size}.out"
                    ...
                    # Run sbatch
                    sbatch_cmd = f"sbatch --job-name={job_name} --output={out} --export=feature_path={feature_path},model={model} slurm_jobs/train_embeddings/training.slurm"
                    print(sbatch_cmd)
