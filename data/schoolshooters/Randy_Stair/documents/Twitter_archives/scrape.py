...
def create_dictionary_of_dfs_from_paths(list_of_files: List[Path] = None, delimiter: str | None = "‎ "):
    ...
    for key, value in paths.items():
        dfs[key] = pd.read_csv(value, delimiter=delimiter, engine='python', encoding="utf-8", quotechar='"', error_bad_lines=False)
    
    return dfs


dict_of_dfs = create_dictionary_of_dfs_from_paths((Path(__file__).parent).rglob("*.csv"), delimiter=",")
df = dict_of_dfs["Andrew Blaze Twitter Archive (EGSandrew)"]
...
# Create file
out = Path(__file__).parent / "jippi.csv"
print(out)
